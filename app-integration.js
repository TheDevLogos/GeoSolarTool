...
(function () {
  ...
  var AUTH_TOKEN_KEY = 'latroy-auth-token';
  var AUTH_PROFILE_KEY = 'latroy-auth-profile';
  ...
  var AUTH_ONLY_MODE = window.LATROY_AUTH_ONLY === true;
  ...
  function authProfile() { try { return JSON.parse(localStorage.getItem(AUTH_PROFILE_KEY) || 'null'); } catch (_) { return null; } }
  function isPublicAction(action) { return action === 'auth_config' || action === 'login'; }
  function storeSession(result) {
    if (result && result.token) localStorage.setItem(AUTH_TOKEN_KEY, result.token);
    if (result && result.user) localStorage.setItem(AUTH_PROFILE_KEY, JSON.stringify(result.user));
  }
  function clearSession() { localStorage.removeItem(AUTH_TOKEN_KEY); localStorage.removeItem(AUTH_PROFILE_KEY); lastDatasetSignature = ''; }
  function emitAuth(name, detail) { document.dispatchEvent(new CustomEvent(name, { detail: detail || {} })); }
  ...
  async function parseResponse(response) {
    ...
    if (json.ok === false) {
      var apiError = new Error(json.error || json.message || 'El backend rechazó la operación.');
      apiError.code = json.code || 'BACKEND_ERROR';
      if (/^AUTH_|FORBIDDEN/.test(apiError.code)) {
        if (/^AUTH_/.test(apiError.code)) { clearSession(); emitAuth('latroy:auth-required', { code: apiError.code, message: apiError.message }); }
      }
      throw apiError;
    }
  ...
  async function request(action, options) {
    ...
    if (method === 'GET') {
      ...
      if (!isPublicAction(action) && authToken()) params.session_token = authToken();
      var query = new URLSearchParams(params);
      targetUrl += (targetUrl.indexOf('?') >= 0 ? '&' : '?') + query.toString();
    } else {
      fetchOptions.headers = { 'Content-Type': 'text/plain;charset=utf-8' };
      fetchOptions.body = JSON.stringify({ action: action, data: options.data || {}, session_token: isPublicAction(action) ? '' : authToken() });
    }
    var response;
  ...
  function configure(url) {
    var previousUrl = configuredUrl;
    configuredUrl = validateUrl(url);
    localStorage.setItem(URL_STORAGE_KEY, configuredUrl);
    if (previousUrl && previousUrl !== configuredUrl) clearSession();
  ...
  async function authConfig(url) { return apiGet('auth_config', {}, url); }
  async function login(loginValue, password) {
    var result = await request('login', { method: 'POST', data: { login: loginValue, password: password } });
    storeSession(result || {}); emitAuth('latroy:auth-session', result || {}); if (!AUTH_ONLY_MODE) { startAutoSync(); sync({ forceDispatch:true }).catch(function(error){ console.error('Error cargando datos después del login:',error); }); } return result;
  }
  async function restoreSession() {
    if (!authToken()) { emitAuth('latroy:auth-required', { code: 'AUTH_REQUIRED' }); return null; }
    try {
      var result = await apiGet('me', {});
      if (result && result.user) localStorage.setItem(AUTH_PROFILE_KEY, JSON.stringify(result.user));
      emitAuth('latroy:auth-session', result || {}); return result;
    } catch (error) { clearSession(); emitAuth('latroy:auth-required', { code: error.code || 'AUTH_REQUIRED', message: error.message }); return null; }
  }
  async function logout() {
    try { if (authToken()) await request('logout', { method: 'POST', data: {} }); } catch (_) {}
    clearSession(); emitAuth('latroy:auth-required', { code: 'LOGOUT' });
  }
  async function changePassword(currentPassword, newPassword) {
    var user = await request('cambiar_password', { method: 'POST', data: { current_password: currentPassword, new_password: newPassword } });
    if (user) localStorage.setItem(AUTH_PROFILE_KEY, JSON.stringify(user));
    emitAuth('latroy:auth-session', { user: user }); return user;
  }
  function listUsers() { return apiGet('usuarios', {}); }
  function saveUser(data) { return request('guardar_usuario', { method: 'POST', data: data || {} }); }
  function resetPassword(userId, password, forceChange) { return request('restablecer_password', { method: 'POST', data: { usuario_id:userId, password:password, forzar_cambio_password: forceChange === false ? 'No' : 'Si' } }); }
  ...
  window.LatroyBackend = {
    ...
    auth: { config:authConfig, login:login, restore:restoreSession, logout:logout, changePassword:changePassword, listUsers:listUsers, saveUser:saveUser, resetPassword:resetPassword, getProfile:authProfile, hasSession:function(){return !!authToken();} }
  };
  ...
  function maybeSyncOnReturn() {
    if (!isConfigured() || !authToken() || document.visibilityState === 'hidden') return;
  ...
  function startAutoSync() {
    ...
    autoSyncTimer = setInterval(function () {
      if (!isConfigured() || !authToken() || document.visibilityState === 'hidden' || !navigator.onLine) return;
      sync({ silent: true }).catch(function () {});
  ...
  window.addEventListener('online', function () {
    if (isConfigured() && authToken()) sync({ silent: true, forceDispatch: true }).catch(function () {});
  });

  document.addEventListener('DOMContentLoaded', function () {
    if (!isConfigured()) {
      emitStatus('unconfigured', 'Configure la URL /exec del backend.');
      emitAuth('latroy:auth-required', { code:'BACKEND_UNCONFIGURED' });
      return;
    }
