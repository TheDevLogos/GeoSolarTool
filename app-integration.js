...
(function () {
  ...
  async function loadCompleteDataset(url) {
    ...
    return Object.assign({}, results[0] || {}, {
      diagnosticos: Array.isArray(results[1]) ? results[1] : [],
      dimensionamientos: Array.isArray(results[2]) ? results[2] : [],
      validaciones: Array.isArray(results[3]) ? results[3] : []
    });
  }
  ...
  async function syncAfterMutation() {
    // Si había una lectura en curso antes de terminar la escritura, esperamos a que
    // concluya y lanzamos una lectura nueva para no importar un estado obsoleto.
    if (syncingPromise) {
  ...
  async function persistAndRefresh(action, payload, savedEventName, localId, successMessage) {
    emitStatus('connecting', 'Guardando y actualizando módulos…');
    var remote = await apiPost(action, payload);
    document.dispatchEvent(new CustomEvent(savedEventName, {
      detail: { localId: localId, remote: remote || {} }
    }));
    // El dispatch anterior es síncrono: CRM.html ya tuvo oportunidad de conservar
    // el backendId antes de volver a importar el conjunto completo.
  ...
  document.addEventListener('latroy:client-saved', function (event) {
    ...
    persistAndRefresh('crear_cliente', {
      ...
      notas: client.notes || ''
    }, 'latroy:client-backend-saved', client.id, 'Cliente guardado y CRM actualizado.').catch(function (error) {
  ...
  document.addEventListener('latroy:diagnostic-saved', function (event) {
    var record = event.detail || {};
    if (!isConfigured() || record.backendId) return;
    persistAndRefresh('crear_diagnostico', {
    ...
    }, 'latroy:diagnostic-backend-saved', record.id, 'Diagnóstico guardado y CRM actualizado.').catch(function (error) {
      emitStatus('error', 'El diagnóstico quedó guardado localmente, pero no en el backend: ' + error.message);
      console.error('Error guardando diagnóstico en backend:', error);
    });
  });

  document.addEventListener('latroy:dimensioning-saved', function (event) {
    var record = event.detail || {};
    if (!isConfigured() || record.backendId) return;
    persistAndRefresh('crear_dimensionamiento', {
      cliente_id: record.backendClientId || record.clientId,
    ...
    }, 'latroy:dimensioning-backend-saved', record.id, 'Dimensionamiento guardado y CRM actualizado.').catch(function (error) {
      emitStatus('error', 'El dimensionamiento quedó guardado localmente, pero no en el backend: ' + error.message);
      console.error('Error guardando dimensionamiento en backend:', error);
    });
  });

  document.addEventListener('latroy:validation-saved', function (event) {
    var record = event.detail || {};
    if (!isConfigured() || record.backendId) return;
    persistAndRefresh('crear_validacion', {
      cliente_id: record.backendClientId || record.clientId,
    ...
    }, 'latroy:validation-backend-saved', record.id, 'Validación guardada y CRM actualizado.').catch(function (error) {
      emitStatus('error', 'La validación quedó guardada localmente, pero no en el backend: ' + error.message);
      console.error('Error guardando validación en backend:', error);
    });
  });
  ...
  document.addEventListener('latroy:quote-saved', function (event) {
    ...
    persistAndRefresh('crear_cotizacion', {
      ...
      detalle: detail,
      roi: roi
    }, 'latroy:quote-backend-saved', quote.id, 'Cotización guardada y CRM actualizado.').catch(function (error) {
