...
(function () {
  ...
  document.addEventListener('latroy:diagnostic-saved', function (event) {
    ...
    apiPost('crear_diagnostico', {
      ...
      document.dispatchEvent(new CustomEvent('latroy:diagnostic-backend-saved', { detail: { localId: record.id, remote: remote || {} } }));
      emitStatus('online', 'Diagnóstico guardado en el backend.');
    }).catch(function (error) {
  ...
  document.addEventListener('latroy:dimensioning-saved', function (event) {
    var record = event.detail || {};
    if (!isConfigured() || record.backendId) return;
    apiPost('crear_dimensionamiento', {
      cliente_id: record.backendClientId || record.clientId,
      ...
      document.dispatchEvent(new CustomEvent('latroy:dimensioning-backend-saved', { detail: { localId: record.id, remote: remote || {} } }));
      emitStatus('online', 'Dimensionamiento guardado en el backend.');
    }).catch(function (error) {
  ...
  document.addEventListener('latroy:validation-saved', function (event) {
    ...
    apiPost('crear_validacion', {
      ...
      document.dispatchEvent(new CustomEvent('latroy:validation-backend-saved', { detail: { localId: record.id, remote: remote || {} } }));
      emitStatus('online', 'Validación guardada en el backend.');
    }).catch(function (error) {
